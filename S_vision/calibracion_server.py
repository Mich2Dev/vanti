...
# -------------------------------------------------------------------------------
FLAG_DB = 25
FLAG_OFFSET = 35  # DB25.DBX34.3 (bool)
...
# (Esta sección se conserva, aunque en tus comentarios dijiste “ya no escribir en CSV”;
# si deseas quitarla, elimina la función guardar_evento y sus llamadas)
# -------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------
CAMERA_DB_MAP = {
    "MR1I":  {"db": 32, "offset": 128, "type": "float"},
    "MR1F":  {"db": 32, "offset": 132, "type": "float"},
    "MR2I":  {"db": 32, "offset": 136, "type": "float"},
    "MR2F":  {"db": 32, "offset": 140, "type": "float"},
    "MR3I":  {"db": 32, "offset": 144, "type": "float"},
    "MR3F":  {"db": 32, "offset": 148, "type": "float"},
    "MR4I":  {"db": 32, "offset": 152, "type": "float"},
    "MR4F":  {"db": 32, "offset": 156, "type": "float"},
    "MR5I":  {"db": 32, "offset": 160, "type": "float"},
    "MR5F":  {"db": 32, "offset": 164, "type": "float"},
    "MR6I":  {"db": 32, "offset": 168, "type": "float"},
    "MR6F":  {"db": 32, "offset": 172, "type": "float"},
    "MR7I":  {"db": 32, "offset": 176, "type": "float"},
    "MR7F":  {"db": 32, "offset": 180, "type": "float"},
    "MR8I":  {"db": 32, "offset": 184, "type": "float"},
    "MR8F":  {"db": 32, "offset": 188, "type": "float"},
    "MR9I":  {"db": 32, "offset": 192, "type": "float"},
    "MR9F":  {"db": 32, "offset": 196, "type": "float"},
    "MR10I": {"db": 32, "offset": 200, "type": "float"},
    "MR10F": {"db": 32, "offset": 204, "type": "float"},
}

PATTERN_DB_MAP = {
    "MP1I":  {"db": 32, "offset": 208, "type": "float"},
    "MP1F":  {"db": 32, "offset": 212, "type": "float"},
    "MP2I":  {"db": 32, "offset": 216, "type": "float"},
    "MP2F":  {"db": 32, "offset": 220, "type": "float"},
    "MP3I":  {"db": 32, "offset": 224, "type": "float"},
    "MP3F":  {"db": 32, "offset": 228, "type": "float"},
    "MP4I":  {"db": 32, "offset": 232, "type": "float"},
    "MP4F":  {"db": 32, "offset": 236, "type": "float"},
    "MP5I":  {"db": 32, "offset": 240, "type": "float"},
    "MP5F":  {"db": 32, "offset": 244, "type": "float"},
    "MP6I":  {"db": 32, "offset": 248, "type": "float"},
    "MP6F":  {"db": 32, "offset": 252, "type": "float"},
    "MP7I":  {"db": 32, "offset": 256, "type": "float"},
    "MP7F":  {"db": 32, "offset": 260, "type": "float"},
    "MP8I":  {"db": 32, "offset": 264, "type": "float"},
    "MP8F":  {"db": 32, "offset": 268, "type": "float"},
    "MP9I":  {"db": 32, "offset": 272, "type": "float"},
    "MP9F":  {"db": 32, "offset": 276, "type": "float"},
    "MP10I": {"db": 32, "offset": 280, "type": "float"},
    "MP10F": {"db": 32, "offset": 284, "type": "float"},
}
...
# (Si ya no deseas escribir en CSV, elimina la función y sus llamadas)
# -------------------------------------------------------------------------------
...
def escribir_bandera_db75(rep: int, valor: bool):
    try:
        data = cliente_plc.db_read(BANDERAS_DB, BANDERAS_BYTE75, 1)
        set_bool(data, 0, rep, valor)
        cliente_plc.db_write(BANDERAS_DB, BANDERAS_BYTE75, data)
        logging.info(f"Escrito bandera en DB{BANDERAS_DB}.DBX{BANDERAS_BYTE75}.{rep} a {valor}")
    except Exception as e:
        logging.error(f"Error escribiendo bandera en DB{BANDERAS_DB}.DBX{BANDERAS_BYTE75}.{rep}: {e}")
...
def procesamiento_dato():
    ...
    # Esta variable se usa para contar las lecturas consecutivas en FALSE de la bandera
    false_count = 0  
    while True:
        if not conectado_plc:
            conectar_plc()
        ...
        if bandera:
            ...
            # (Si se utiliza CSV, se guardan los eventos iniciales)
            ...
            # Activar bandera de repetición 1 (por ejemplo, DBX34.5 se pone en TRUE)
            escribir_bandera_db74(5, True)
            escribir_valor_plc("MR1I", valor_inicial_camara, cliente_plc)
            escribir_valor_plc("MP1I", v_patron_inicial, cliente_plc)
            
            # Generar umbrales para cada repetición
            thresholds = []
            thresholds.append(valor_inicial_camara + setpoint)  # Umbral para repetición 1
            for i in range(1, 10):
                thresholds.append(thresholds[i-1] + GAP)
            
            # Inicializar arrays de inicio y final y estados
            ...
            while True:
                # Se revisa la bandera externa; se hará la verificación 3 veces consecutivas antes de abortar
                if not leer_bandera_test():
                    ...
                    false_count = 0  # reiniciar el contador si la bandera vuelve a TRUE
                ...
                if NUM_REPETICIONES >= 10:
                    ...
                    if rep_inicio_registrado[9] and (not rep_final_registrado[9]) and (v_camara >= thresholds[9]):
                        ...
                        escribir_bandera_db75(6, False)
                        logging.info("Repetición 10 final: MR10F=%.2f, MP10F=%.2f | difCam=%.2f, difPat=%.2f",
                                     r_final_camara[9], r_final_patron[9], dif_cam, dif_pat)

                # Si todas las repeticiones configuradas han finalizado, apaga la bandera general,
                # desconecta el PLC y sale del bucle de calibración.
                if all(rep_final_registrado[:NUM_REPETICIONES]):
                    escribir_bandera_test(False)
                    cliente_plc.disconnect()
                    conectado_plc = False
                    logging.info("Fin de calibración. Se apaga la bandera general y se cierra la conexión al PLC.")
                    break
